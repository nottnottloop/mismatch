...
func _process(delta: float) -> void:
	if i <= len(finishedText)-1 and Time.get_ticks_msec()-lastLetter> 50/Engine.time_scale and finishedText:
		if finishedText[i]  in ",.-":
		...
		text = "[shake  rate=4 level=8]"+finishedText.substr(0,i+1)+"[/shake]"
		i += 1
